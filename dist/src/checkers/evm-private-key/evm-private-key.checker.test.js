...
function generateEthereumPrivateKeys(numberKeys) {
    const keys = [];
...
function generateEthereumPrivateKeysWithPrefix(numberKeys) {
    const keys = generateEthereumPrivateKeys(numberKeys);
    return keys.map((key) => `0x${key}`);
}
function generateRandomStrings(numberKeys) {
    const keys = [];
    const characters = 'abcdefghijklmnopqrstuvwxyz0123456789';
    const keyLength = 100;
    for (let index = 0; index < numberKeys; index++) {
        keys.push(Array.from({ length: keyLength }, () => characters.charAt(Math.floor(Math.random() * characters.length))).join(''));
    }
    return keys;
}
describe('EVM checker', () => {
    let checker;
    beforeAll(() => {
        checker = new evm_private_key_checker_1.EvmPrivateKeyChecker();
    });
    test('should filter one private key', async () => {
        const testDataInput1 = 'test 0x10407901cb518b309f733be88bd01d6083d00fe9e7e60e9fb2cc87ad3a415a1f test';
        const testDataOutput1 = 'test <Sensitive data was removed by EVM private key checker> test';
        ...
        const testDataInput2 = 'test 10407901cb518b309f733be88bd01d6083d00fe9e7e60e9fb2cc87ad3a415a1f test';
        const testDataOutput2 = 'test <Sensitive data was removed by EVM private key checker> test';
    ...
    test('should filter multiple private keys', async () => {
        const testDataInput1 = 'test 0x10407901cb518b309f733be88bd01d6083d00fe9e7e60e9fb2cc87ad3a415a1f test 0x20407901cb518b309f733be88bd01d6083d00fe9e7e60e9fb2cc87ad3a415a1f';
        const testDataOutput1 = 'test <Sensitive data was removed by EVM private key checker> test <Sensitive data was removed by EVM private key checker>';
        const result1 = await checker.processData(testDataInput1);
        expect(result1.valueOut).toEqual(testDataOutput1);
        expect(result1.triggered).toBe(true);
    });
    ...
    test('should filter many strings', async () => {
        ...
        const testDataB = generateEthereumPrivateKeysWithPrefix(CONST_NUMBER_OF_TESTS);
        const testData = [
            ...testDataA,
            ...testDataB,
...
//# sourceMappingURL=evm-private-key.checker.test.js.map